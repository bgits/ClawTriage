...
export class Storage {
  ...
  async finishAnalysisRun(
    ...
    await this.pool.query(
      ...
      `,
      [analysisRunId, status, error ?? null, degradedReasons ?? null],
    );
  }
  ...
  async listTriageQueue(params: {
    ...
    const { repoId, state, needsReview, limit, cursor, reviewThreshold } = params;
    ...
    if (cursor) {
      ...
      cursorClause = `
        AND (pr.updated_at, pr.pr_id) < ($5::timestamptz, $6::bigint)
      `;
    }
    ...
    const result = await this.pool.query(
      ...
      WHERE pr.repo_id = $1
        ...
        ${needsReviewClause}
        ${cursorClause}
      ORDER BY pr.updated_at DESC, pr.pr_id DESC
      LIMIT $4
      `,
      queryValues,
    );
    ...
    const items = selectedRows.map((row) => {
      ...
      return {
        ...
        analysisStatus: String(row.analysis_status) as
          ...
          | "FAILED",
        topSuggestion:
          row.top_category && row.candidate_pr_id && row.candidate_pr_number && score !== null
            ? {
    ...
    return {
      items,
      nextCursor: hasMore && last ? encodeCursor(last.updatedAt, last.prId) : null,
    };
  }
}
